...
Env noise level increases linearly with env id, from 0 up to ``noise_std``.
...
Trajectory list layout (deterministic):
    ...
    trajectories[1..N-1]      -> env-1..N-1, linearly increasing noise
...
parser.add_argument("--num_envs", type=int, default=4096, help="Number of parallel environments (env-0 is noise-free, later envs use larger noise).")
...
parser.add_argument("--noise_std", type=float, default=0.05, help="Maximum action-space Gaussian noise std (linearly reached at the largest env id).")
...
def _build_noise_mask(num_envs: int, noise_std: float, device: torch.device) -> torch.Tensor:
    """Per-env noise scale increases linearly with env id from 0 to noise_std."""
    if num_envs <= 1:
        return torch.zeros((num_envs, 1), device=device)
    mask = torch.linspace(0.0, noise_std, steps=num_envs, device=device).unsqueeze(-1)
    return mask
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # Export should follow play-style uninterrupted rollout and never auto-reset
    # in the middle of a clip. Disable perturbations and all termination terms.
    ...
    max_steps_arg = getattr(args_cli, "max_steps", None)
    max_steps = max_steps_arg if max_steps_arg is not None else default_max_steps

    noise_std = getattr(args_cli, "noise_std", 0.05)
    ...
    env_noise_levels = noise_mask.squeeze(-1).detach().cpu()
    ...
    print_dict(
        {
            ...
            "max_steps": max_steps,
            "noise_std": noise_std,
            "noise_schedule": f"linear_by_env_id: 0 -> {float(env_noise_levels[-1]) if len(env_noise_levels) > 0 else 0.0}",
            "task": task_name,
    ...
    if num_envs > 1:
        print(
            ...
            f"env{num_envs-1}={env_noise_levels[-1].item():.6f}"
        )
    ...
    for env_id in range(num_envs):
        ...
        env_noise = float(env_noise_levels[env_id].item())
        traj["metadata"] = {
            "total_frames": steps,
